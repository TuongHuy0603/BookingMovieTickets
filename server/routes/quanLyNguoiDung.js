...
router.post("/ThongTinTaiKhoan", async (req, res) => {
  ...
  try {
    ...
    const bookTickets = await BookTicket.find({
    ...
    for (let x = 0; x < bookTickets.length; x++) {
      ...
      const listSeatBooks = await ListSeatBook.find({
        maDatVe: element._id,
      }).populate({
        path: "maGhe",
        populate: {
...
router.delete("/XoaNguoiDung", verifyTokenAdmin, async (req, res) => {
  ...
  try {
    ...
    const userdeleted = await User.findOneAndDelete({ taiKhoan: taiKhoan });
